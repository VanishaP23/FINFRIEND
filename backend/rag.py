...
def build_index():
    ...
    texts = sorted(glob.glob(os.path.join(DOCS_DIR, "**", "*.txt"), recursive=True) +
                   glob.glob(os.path.join(DOCS_DIR, "**", "*.md"),  recursive=True))
    texts = [t for t in texts if "readme" not in os.path.basename(t).lower()]  # skip READMEs
    if not pdfs and not texts:
        print(f"No PDF/TXT/MD files found under {DOCS_DIR}. Drop your documents there first.")
        return
    ...
    # --- TEXT / MARKDOWN (recursive, from topic subfolders) ---------------
    # One "## heading" section = one retrievable idea. The section number is
    # used as the citation "page", so a hit still renders as source + page.
    for txt in texts:
        tag, fname = _source_label(txt), os.path.basename(txt)
        with open(txt, encoding="utf-8") as f:
            raw = f.read()
        sections = [s for s in raw.split("\n## ") if len(s.strip()) >= 40]
        for secno, sec in enumerate(sections, start=1):
            for piece in _chunk_text(sec.strip()):
                piece = piece.strip()
                if len(piece) < 40:
                    continue
                docs.append(piece)
                metas.append({"source": tag, "file": fname, "page": secno})
                ids.append(f"{fname}-s{secno}-{n}")
                n += 1
    ...
    print(f"Indexed {len(docs)} chunks from {len(pdfs)} PDF(s) + {len(texts)} text file(s) into {CHROMA_DIR}")
    print("Sources:", ", ".join(sorted({_source_label(p) for p in pdfs + texts})))
